...
difference(){
    color("red")translate([0,-130,-case_z])linear_extrude(case_z+pcb_z)offset(r=3)import("rounded.svg");

    color("red")translate([0,-130,-0.])linear_extrude(pcb_z)offset(r=0.5)import("rounded.svg");
    translate([5*hex_x,-6*hex_y,-(case_z+pcb_z)])linear_extrude(case_z+pcb_z)import("center.svg"); //cutout for graphics
    translate([6.5*hex_x,-2.5*hex_y,1.5])minkowski(){cube([hex_x-10,30,2],center=true);sphere(1);} //mcu
...
translate([+3.5*hex_x,-3*hex_y,0])choc_hs();
translate([+4*hex_x,-4*hex_y,0])choc_hs();
translate([+6.5*hex_x,-3*hex_y,0])choc_hs();
translate([+7.5*hex_x,-3*hex_y,0])choc_hs();
translate([+6*hex_x,-4*hex_y,0])choc_hs();
}

translate([drill_x,drill_y,drill_z]){
    cylinder(drill_h,r=drill_r);
    translate([-0.5*hex_x,-hex_y,0])cylinder(drill_h,r=drill_r);
    translate([+3*hex_x,-2*hex_y,0])cylinder(drill_h,r=drill_r);
    translate([+10*hex_x,0,0])cylinder(drill_h,r=drill_r);
    translate([+10.5*hex_x,-hex_y,0])cylinder(drill_h,r=drill_r);
    translate([+7*hex_x,-2*hex_y,0])cylinder(drill_h,r=drill_r);
}
// bumbers
    bumper_h=0.5;
    bumper_d=8.5;
    ...
    translate([-(9.75-base_x/2-5),-(-6.9+base_y/2-3.8),-case_z])translate([first_x,first_y,first_z]){
        ...
        translate([10*hex_x,0,0]+top_shift)bumper(h=bumper_h,d=bumper_d,num=1,ht=marker_h);

        translate([4*hex_x,0,0]+top_shift)bumper(h=bumper_h,d=bumper_d,num=3,ht=marker_h);
        translate([6*hex_x,0,0]+top_shift)bumper(h=bumper_h,d=bumper_d,num=3,ht=marker_h);
        translate([4*hex_x,-4*hex_y,0]-top_shift)bumper(h=bumper_h,d=bumper_d,num=3,ht=marker_h);
        translate([6*hex_x,-4*hex_y,0]-top_shift)bumper(h=bumper_h,d=bumper_d,num=3,ht=marker_h);

        translate([-1*hex_x,-2*hex_y,0]+corner_shift_l-top_shift)bumper(h=bumper_h,d=bumper_d,num=2,ht=marker_h);
        translate([11*hex_x,-2*hex_y,0]+corner_shift_r-top_shift)bumper(h=bumper_h,d=bumper_d,num=2,ht=marker_h);
